...
import proyecto.umg.crud.BancoEdit;
import proyecto.umg.crud.BancoNew;
...
public class MovimientosCuentaRpt extends ViewBase implements ClickListener,
		ValueChangeListener {
	...
	private List<ChkMovimientosCuenta> listaItems;
	private ChkMovimientosCuenta selected;
	
	
	
	
	public MovimientosCuentaRpt(){
	...
	public void obtenerListaItems(){
		...
		ProjectDao<ChkMovimientosCuenta> dao = null;
		try {
			
			if (fechaInicial.getValue() == null && fechaFinal.getValue() == null){
				dao =new ProjectDao<ChkMovimientosCuenta>(new ChkMovimientosCuenta());
				listaItems = dao.findAll("ChkMovimientosCuenta.findAll");
			}else if (fechaInicial.getValue() != null && fechaFinal.getValue() != null){
				dao =new ProjectDao<ChkMovimientosCuenta>(new ChkMovimientosCuenta());
				listaItems = dao.findElements("SELECT c FROM ChkMovimientosCuenta c "
						+ "where c.fechaMovimiento between ?1 and ?2",
			...
			listadoItems.addContainerProperty("Tipo Movimiento", Object.class, "");
			listadoItems.addContainerProperty("Fecha", Object.class, "");
			listadoItems.addContainerProperty("Monto", Object.class, null);
			
			listadoItems.addContainerProperty("Documento Banco", Object.class, null);
			int fila = 1;
			if (listaItems == null){
			...
			for (ChkMovimientosCuenta u: listaItems){
				
				listadoItems.addItem(new Object[]{u.getChkCuenta().getNumeroCuenta(),
						u.getTipoMovimiento().intValue() == 1 ? "DEPOSITO" : "CHEQUE", 
								formatearFecha(u.getFechaMovimiento()), 
									u.getChkCuenta().getMoneda()+". "+u.getMontoMovimiento().setScale(2, RoundingMode.CEILING),
										u.getDocumentoBanco()}, fila);
				fila++;				
			}			
	...
	public void buildMainLayout() {
		...
		listadoItems = new Table("Movimientos de Cuenta");
		obtenerListaItems();
		...
		setStyleButtons(btnPrint, generarReporte, btnBuscar);
		principal.addComponent(lyBotones);
		
	}
	private void generarReporte() throws Exception {
		// TODO Auto-generated method stub
		String archivo = "reporte_movimientos.xls";
